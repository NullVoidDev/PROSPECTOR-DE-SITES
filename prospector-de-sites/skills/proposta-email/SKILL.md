...
- [ ] **Link como âncora HTML com texto visível limpo.** O Gmail embrulha TODO link em um redirect próprio (`google.com/url?q=...`) ao salvar — não dá pra impedir, e em corpo de texto puro o embrulho fica VISÍVEL, o que parece golpe. Por isso o rascunho é criado com corpo HTML e o link como âncora: `<a href="https://[projeto-demo].vercel.app/proposta.html">https://[projeto-demo].vercel.app/proposta.html</a>` — texto visível = a URL limpa montada a partir do deploy (nunca copiada de outro e-mail). O redirect do Google fica só no href invisível, como em qualquer e-mail do Gmail. Depois de criar, confira o rascunho: o texto visível deve começar em `https://` do deploy.
- [ ] **Domínio limpo e humano.** O deploy padrão da Vercel gera um subdomínio (ex: `https://demo-[slug].vercel.app`). Caso o usuário possua domínio próprio customizado para as demonstrações, oriente a usá-lo.
- [ ] **Sem palavras-gatilho**: grátis, promoção, imperdível, oferta, desconto, clique aqui, 100%, garantido, urgente.
- [ ] **Sem CAIXA ALTA no assunto, sem "!!" / emoji** no assunto.
